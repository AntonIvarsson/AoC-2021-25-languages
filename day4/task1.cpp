...
int winner_board(int (&scores)[BOARDS][5][5]) {
    for (int i=0; i < BOARDS; i++) {
      if (is_winner_board(scores[i]))
        return i;
    }
...
int main()
{
  ...
  for(int i = 0; i < numbers.size(); i++) {
      ...
      int winnerBoard = winner_board(scores);
      if (winnerBoard == -1) {
          continue;
      } else {
        show_board(scores[winnerBoard], boards[winnerBoard]);
        cout << "winner: " << winnerBoard << endl;
        cout << "score: " << calculate_score(scores[winnerBoard], boards[winnerBoard], number);
